...
def create():
    ...
    token = Auth.generate_token(ser_data.get('id'))
    user = {}
    user.update({'id': ser_data.get('id'), 'name': ser_data.get('name'), 'email':ser_data.get('email'), 'phone': ser_data.get('phone')})
    return custom_response({'token':token, 'user':user},201)
